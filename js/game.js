...
// Function to create the Quick Solve button
const createQuickSolveButton = () => {
  ...
  startButton.addEventListener("click", () => {
    ...
    movesCount = 0;
    moves.innerText = `Moves: ${movesCount}`;
    createQuickSolveButton(); // Create the Quick Solve button after Start Game is clicked
  });
...
// Function to solve the puzzle quickly
const quickSolvePuzzle = () => {
    if (!isPuzzleSolved()) {
      // Rearrange the imagesArr to a solved state
      imagesArr = [1, 2, 3, 4, 5, 6, 7, 8, 9];
      // Regenerate grid with solved arrangement
      container.innerHTML = "";
      gridGenerator();
      movesCount = 0;
      moves.innerText = `Moves: ${movesCount}`;
      // Display the number of moves it took to quick solve
  ...
  // Event listener for the quick solve button
